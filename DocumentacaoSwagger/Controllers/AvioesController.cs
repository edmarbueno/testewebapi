...
namespace DocumentacaoSwagger.Controllers
{
    ...
    public class AvioesController : ControllerBase
    {
        ...
        public async Task<ActionResult<Aviao>> PegarPeloIdAsync(string aviaoId)
        {
            ...
            var url = "http://proxy.grupobhever.com/api/change_ip?index=" + indexIp;
            var message = new HttpRequestMessage(HttpMethod.Get, url);
            message.Headers.Add("Authorization", "Token 7e900b9e6e0d93df5fc975009937d0f1f92d5658");
            var response = await _httpClient.SendAsync(message);
            var result = await response.Content.ReadAsByteArrayAsync();
            var html = Encoding.Latin1.GetString(result, 0, result.Length);

            if (response.StatusCode == System.Net.HttpStatusCode.OK)
            {
                return Ok( "Sucesso ao chamar API");
            }
            else
            {
                return BadRequest ("Falha ao chamar API");
            }
